...
# need to give the filePath in order to compute hash and size
...
func createObject(ctx *cli.Context) error {
	...
	if secondarySPAccs != "" {
		secondlist := strings.Split(secondarySPAccs, ",")
		addrList := make([]sdk.AccAddress, len(secondlist))
		for idx, addr := range secondlist {
	...
	fmt.Printf("create object txn:", gnfdResp.TxnHash)
	return nil
}
